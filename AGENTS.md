...
| `docs/` | Empty letterhead. Do not dump a deliverable into these. |
| `examples/` | **Vibe specimens.** Study, don’t clone. See `examples/README.md`. |
...
For a written deliverable: **design the layout for this content.** Study `examples/` for vibe (proportion, 8px/2px, type, nested accents). Use `DESIGN.md` components. Two docs in the same genre do not have to share a skeleton.

**Never** paste into `docs/01-branded.html` or `docs/02-working.html`. That’s the failure mode. Those files are letterhead, not a document.
